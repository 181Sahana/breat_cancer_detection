...
class Ensemble:
    ...
    def linearRegression(self):
        lin_reg = pickle.load(
            open(self.path.models_path + self.path.lin_reg_path, 'rb'))
    ...
    def logisticRegression(self):
        log_reg = pickle.load(
            open(self.path.models_path + self.path.log_reg_path, 'rb'))
    ...
    def RandomForest(self):
        random_forest = pickle.load(
            open(self.path.models_path + self.path.rf_classifer_reg_path, 'rb'))
        pred = random_forest.predict(self.data)
        self.ensemble.extend(pred)

    def SVC(self):
        svc = pickle.load(
            open(self.path.models_path + self.path.svc_path, 'rb'))
